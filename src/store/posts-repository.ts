...
/** Triage rating a reviewer gives a post. Empty string means not yet rated. */
export type PostVerdictRating = '' | 'yes' | 'maybe' | 'no';

export const VERDICT_RATINGS: PostVerdictRating[] = ['', 'yes', 'maybe', 'no'];
...
export interface DbPost {
  ...
  salary: string;  // Normalised pay inferred from post text, '' when absent
  verdict: PostVerdictRating; // Triage rating, '' until rated
}
...
export async function insertPost(
  ...
  country: string = '',
  salary: string = '',
  verdict: PostVerdictRating = ''
): Promise<number | null> {
  const db = await getDatabase();
  
  try {
    db.run(
      `INSERT INTO posts (search_keywords, post_link, description, search_date, applied, profile_image, author_name, author_occupation, post_date, like_count, comment_count, country, salary, verdict)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        keywords, 
        link, 
...
export async function countPosts(params?: {
  ...
  if (params) {
    ...
    // Filter by triage rating ('' selects posts that are not yet rated)
    if (params.verdict !== undefined) {
      sql += ` AND COALESCE(verdict, '') = ?`;
      bindings.push(params.verdict);
    }
    
    // Filter by country (substring match - country holds a comma-separated list)
    if (params.country) {
      sql += ` AND country LIKE ?`;
      bindings.push(`%${params.country}%`);
    }
    
    // Filter by whether the post quotes any pay
    if (params.hasSalary !== undefined) {
      sql += params.hasSalary ? ` AND salary != ''` : ` AND salary = ''`;
    }
  }
...
export async function updatePostsBulk(
  ...
  if (updates.new_verdict !== undefined) {
    setClauses.push('verdict = ?');
    bindings.push(updates.new_verdict);
  }
...
/**
 * Set the triage rating for a post.
 * Returns true if updated, false if the post does not exist.
 */
export async function setVerdict(id: number, verdict: PostVerdictRating): Promise<boolean> {
  const db = await getDatabase();
  ...
  db.run('UPDATE posts SET verdict = ? WHERE id = ?', [verdict, id]);
  saveDatabase();
  return true;
}

/**
 * Count posts per triage rating, for dashboard summaries.
 */
export async function countByVerdict(): Promise<Record<PostVerdictRating, number>> {
  const db = await getDatabase();
  const tally: Record<PostVerdictRating, number> = { '': 0, yes: 0, maybe: 0, no: 0 };
  
  const result = db.exec("SELECT COALESCE(verdict, '') AS v, COUNT(*) FROM posts GROUP BY v");
  if (result.length === 0) {
    return tally;
  }
  
  for (const [rating, count] of result[0].values) {
    const key = (rating ?? '') as PostVerdictRating;
    if (key in tally) tally[key] = count as number;
  }
  return tally;
}
