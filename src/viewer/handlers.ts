...
/**
 * PATCH /api/posts/:id/verdict - Set the triage rating for a post
 */
export async function handleSetVerdict(req: Request, res: Response): Promise<void> {
  try {
    const id = parseInt(req.params.id);
    ...
    if (typeof verdict !== 'string' || !allowed.includes(verdict)) {
      res.status(400).json({ error: "verdict must be one of '', 'yes', 'maybe', 'no'" });
      return;
    }
    
    const updated = await setVerdict(id, verdict as '' | 'yes' | 'maybe' | 'no');
    ...
    res.status(500).json({ error: 'Failed to set verdict' });
  }
}
