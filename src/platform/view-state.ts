...
import { getDataDirectory } from './workspace-paths.js';
...
export interface FilterState {
  ...
  /** Triage rating: 'all', or one of the ratings, or 'unrated'. */
  verdictFilter: 'all' | 'yes' | 'maybe' | 'no' | 'unrated';
  countryFilter: string;    // Empty string means all countries
  payFilter: 'all' | 'with-pay';
}
...
export function updateFilterState(updates: Partial<FilterState>): FilterState {
  ...
  // Validate verdictFilter enum
  if (updates.verdictFilter && !['all', 'yes', 'maybe', 'no', 'unrated'].includes(updates.verdictFilter)) {
    throw new Error(`Invalid verdictFilter value: ${updates.verdictFilter}`);
  }
  
  // Validate payFilter enum
  if (updates.payFilter && !['all', 'with-pay'].includes(updates.payFilter)) {
    throw new Error(`Invalid payFilter value: ${updates.payFilter}`);
  }
